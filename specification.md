...
- **Cache invalidation:** `POST /api/cache/invalidate` removes cached tiles inside a bounding box (defined by `south`, `west`, `north`, `east`) when the supplied `secret` matches `CACHE_INVALIDATION_SECRET`. The endpoint returns a JSON summary of deleted keys and affected amenities.
