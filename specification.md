...
- **Statistics + time helpers:** `src/stats.ts` tracks per-amenity demand, cache status, cache coverage, and per-amenity geohash coverage for the `/api/statistics`, `/api/statistics/cacheCoverage`, and `/api/statistics/geohashCoverage` endpoints, while `src/time.ts` exposes day-boundary helpers shared by statistics and upstream quota logic.
...
4. **Stale tracking:** Tiles past their TTL are marked `stale`. When `SERVE_STALE_FROM_CACHE=true` and the request is fully satisfied by cached tiles, stale data is returned immediately and refreshed asynchronously via per-tile locks (SWR window `SWR_SECONDS`). When the request contains cache misses or `SERVE_STALE_FROM_CACHE=false`, stale tiles are refreshed before replying so the client receives the latest attempted write.
5. **Upstream fetch:** Missing tiles are fetched individually by issuing the canonical Overpass multi-entity bbox query (`node`, `way`, `relation`) via POST `application/x-www-form-urlencoded`, scoped to the specific amenity type supplied by the client. If upstream fetches fail and missing tiles remain unresolved, the proxy returns HTTP 503 with cache metadata headers rather than serving partial stale data.
...
- Non-interpreter `/api/*` endpoints (`/api/status`, `/api/timestamp`, `/api/timestamp/*`, `/api/kill_my_queries`, `/api/statistics`, `/api/statistics/cacheCoverage`, `/api/statistics/cacheCoverage/area`, and arbitrary paths) are proxied verbatim when caching is disabled or the request falls outside amenity JSON bbox handling.
- `/api/statistics`, `/api/statistics/cacheCoverage`, `/api/statistics/cacheCoverage/area`, and `/api/statistics/geohashCoverage` are served locally, returning aggregated JSON metrics derived from the Redis-backed `RequestStatistics` service so data survives restarts.
...
- `CACHE_INVALIDATION_SECRET` (when set, enables `POST /api/cache/invalidate` to purge cached tiles inside a bbox)
...
## 11. Request Statistics and Operations Dashboards
...
- **Aggregation:** `getSnapshot` produces totals for requests, tiles, unique clients, cache status counts with a hit rate, cache inventory (total cached tiles and amenities), and per-amenity breakdowns including cache inventory, cache hit rates, geohash coverage (precision 4), last-request timestamps, and average tile counts. The service also highlights the top ten geohash hotspots across all amenities and preserves the persistence boundary in Redis. `getCacheCoverageSnapshot` returns the cached tile coverage snapshot, sorted by tile entries for `/api/statistics/cacheCoverage`, and `getGeohashCoverageSnapshot` returns per-amenity geohash coverage shares without cache inventory payloads for `/api/statistics/geohashCoverage`.
- **Endpoint:** `/api/statistics` returns the current snapshot as JSON, enabling external systems to monitor demand and tune cache pre-warming or upstream routing strategies with data that survives process restarts. `/api/statistics/cacheCoverage` serves the cached tile coverage separately for consumers that only need inventory detail, `/api/statistics/cacheCoverage/area` returns cache coverage scoped to a bbox at a requested geohash precision, and `/api/statistics/geohashCoverage` exposes per-amenity coverage percentages for viewers that only need hotspot context.
- **Operations dashboards:** `public/statistics-map.html` renders request hotspots, request coverage, and cache coverage overlays with progress feedback, deduplicated geohash entries, stale coverage colouring, and toggles for refreshing data or collapsing the upstream health panel. `public/cache-preheater.html` targets manual cache warmups with live progress and batching safeguards to reduce browser memory pressure during large runs. `public/cache-invalidator.html` provides a map UI for drawing a bounding box and submitting cache invalidation requests.
- **Cache invalidation:** `POST /api/cache/invalidate` removes cached tiles inside a bounding box when the supplied `secret` matches `CACHE_INVALIDATION_SECRET`. Bounding boxes may be provided as `bbox=south,west,north,east`, as a JSON array, or via `south`, `west`, `north`, `east` fields in the query string or JSON body. The endpoint returns `ok`, the resolved `bbox`, `tileCount`, and a summary of `deletedKeys`, `matchedKeys`, `tileHashes`, and `affectedAmenities`.
